...
{
    SetupDial(dial, settings.driver);
...
void RoundWatch::SetupDial(const DIAL dial, const Driver::Settings& driverSettings)
{
    if (!m_dials[INT(dial)])
        m_dials[INT(dial)] = RoundDial::CreateDial(dial, driverSettings);
}
